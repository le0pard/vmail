...
<script>
  ...
  $: {
    if ($isDarkThemeON !== isDarkThemeActive) {
      theme.switchTheme(isDarkThemeActive)
      activateTheme($theme)
    }
  }
</script>
...
<label class="toggle-wrapper" for="theme-toggle">
  <div class="toggle" class:toggle__dark="{isDarkThemeActive}">
    ...
    <input
      ...
      name="theme-toggle"
      type="checkbox"
      bind:checked="{isDarkThemeActive}"
    />
  </div>
</label>
